...
const sendMessage = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      // Don't add the full response - status updates are already shown during process
      // Just keep the progress updates as the final message
      messages.value[assistantMessageIndex].content = existingProgress || fullText;
    } else {
